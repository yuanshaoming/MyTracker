...
`tracker_replay` 通过固定 CSV 重放 Task 06 的最小 SORT，不读取图片、检测模型或 GUI 数据。

```sh
tracker_replay --frames <frames.csv> --detections <detections.csv> --output <tracks.csv>
```

工具使用默认 `TrackerConfig`。输入必须为带表头的 UTF-8 数值 CSV；格式不支持引号、转义逗号或电子表格公式。
...
每行对应某个输入帧中 `SortTracker::update()` 返回的一条轨迹。无轨迹帧没有结果行，但仍由 `frames.csv` 保留。状态固定为 `Tentative`、`Confirmed` 或 `Lost`；记录按帧表顺序和 `track_id` 升序输出。浮点数固定为 6 位小数，使用小数点，保证同环境同输入可逐字节比较。
